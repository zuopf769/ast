...
const babelImportPlugin = opts => {
  return {
    visitor: {
      ImportDeclaration(path) {
        const { node } = path // 获取节点
        const { specifiers } = node // 获取批量导入声明数组
        const { libraryName, libraryDirectory = 'lib' } = opts // 获取选项中的支持的库的名称
        // 如果当前的节点的模块名称是我们需要的库的名称
        if (
          node.source.value === libraryName &&
          // 并且导入不是默认导入才会进来，因为默认导入就是想全部导入，只能依赖treeshaking，
          // treeshaking和按需加载是两回事
...
module.exports = babelImportPlugin
